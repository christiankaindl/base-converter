...
var Base = {
  convert(from, targetBase, number) {
    ...
    if (!Base.validateNumber(number, from)) return false;

    if (from !== 10) number = toBaseTen(number, from);

    for (let i in targetBase) results[i] = fromBaseTen(number, targetBase[i]);
    ...
    function toBaseTen(number, base) {
      ...
      if (number.toString().split(",")[1]) calcFractionPart();

      return sum.toString();

      function calcIntegerPart() {
        var numberIntDigits = number
          .toString()
          .split(",")[0]
          .split("");
        ...
        for (let i in numberIntDigits)
          sum +=
            Base.getDigitValue(numberIntDigits[i]) *
            Math.pow(base, numberIntDigits.length - 1 - i);
      }
      ...
      function calcFractionPart() {
        ...
        for (let i in numberFractionDigits)
          temp +=
            Base.getDigitValue(numberFractionDigits[i]) *
            Math.pow(base, -(i + 1));
  ...
  validateNumber(number, base = 10) {
    ...
    for (let i in digits) {
      ...
      if (digits[i] === "," && isComma === false) isComma = true;

      if (digits[i] === "," && isComma === true) return false;
